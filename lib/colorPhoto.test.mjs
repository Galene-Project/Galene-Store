...
test('validateColorPhotos remove entradas com photo_url vazio (limpar foto)', () => {
  const result = validateColorPhotos([
    { color_id: 'c1', photo_url: '' },
    { color_id: 'c2', photo_url: '   ' },
    { color_id: 'c3', photo_url: 'https://exemplo.com/foto.jpg' },
  ]);
  assert.deepEqual(result, [{ color_id: 'c3', photo_url: 'https://exemplo.com/foto.jpg' }]);
});
...
test('validateColorPhotos com entrada não-array devolve lista vazia', () => {
  assert.deepEqual(validateColorPhotos(undefined), []);
});

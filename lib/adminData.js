...
import { comparativoMensal, rankClientes, produtosParados as calcProdutosParados, lucroLiquido, pontoEquilibrio } from './dashboardMetrics';
...
export async function fetchDashboardData() {
  const [ordersRes, itemsRes, stockRes, productsRes, custoRes, expensesRes, recurringRes, productionRunsRes] = await Promise.all([
    supabase.from('orders').select('id, order_number, status, total, created_at, paid_at, customer_id, customers(name, company_name)'),
    ...
    supabase.from('products').select('id, custo'),
    supabase.from('expenses').select('id, categoria, subcategoria, valor, data_competencia, data_pagamento').order('data_competencia', { ascending: false }).limit(5000),
    supabase.from('recurring_expenses').select('valor').eq('ativo', true),
    supabase.from('production_runs').select('id, custo_total, data, products(name)').order('data', { ascending: false }).limit(2000),
  ]);
  ...
  const custoPorProdutoId = new Map((custoRes.data || []).map((p) => [p.id, Number(p.custo) || 0]));
  const expenses = expensesRes.data || [];
  const despesaFixaMensal = (recurringRes.data || []).reduce((s, r) => s + Number(r.valor), 0);
  const productionRuns = productionRunsRes.data || [];
  ...
  const janela6Meses = last6Months();
  const inicioJanela = new Date(janela6Meses[0].year, janela6Meses[0].month, 1);
  const orderIdsJanela = new Set(orders.filter((o) => new Date(o.created_at) >= inicioJanela).map((o) => o.id));

  const cogsJanela = paidItems
    .filter((i) => orderIdsJanela.has(i.order_id))
    .reduce((s, i) => s + i.quantity * (custoPorProdutoId.get(i.product_id) || 0), 0);

  const despesasJanela = expenses
    .filter((e) => {
      const [y, m] = e.data_competencia.split('-').map(Number);
      return janela6Meses.some((j) => j.year === y && j.month === m - 1);
    })
    .reduce((s, e) => s + Number(e.valor), 0);

  const faturamentoJanela = faturamentoMensal.reduce((s, m) => s + m.valor, 0);
  const lucroLiquidoValor = lucroLiquido({ faturamento: faturamentoJanela, cogs: cogsJanela, despesas: despesasJanela });
  const equilibrio = pontoEquilibrio(despesaFixaMensal, faturamentoMensal[5].valor);
  ...
  return {
    ...
    estoquePorCategoria,
    raw: { orders, paidItems, custoPorProdutoId, expenses, despesaFixaMensal, productionRuns },
    kpis: {

...
import { validateInstagramUrl, validateInstagramUrls, sortVideoFirst } from './instagramUrl.js';
...
test('sortVideoFirst coloca reel antes de post de foto', () => {
  const result = sortVideoFirst([
...
test('sortVideoFirst com só posts ou só reels mantém ordem', () => {
  assert.deepEqual(sortVideoFirst(['https://www.instagram.com/p/AAA/', 'https://www.instagram.com/p/BBB/']),

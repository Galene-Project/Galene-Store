...
test('agrupa por cor x tamanho, somando peças e receita de múltiplos pedidos', () => {
  ...
  const result = productReport(items, orders, { productId: 'p1' });
...
test('pedido aguardando_aprovacao (cartão pendente) não entra na soma', () => {
  const orders = [
    order({ id: 'o1', status: 'aguardando_aprovacao' }),
    order({ id: 'o2', status: 'pago' }),
  ];
  const items = [
    item({ order_id: 'o1', quantity: 6 }),
    item({ order_id: 'o2', quantity: 2 }),
  ];
  const result = productReport(items, orders, { productId: 'p1' });
  assert.equal(result.totalPecas, 2);
});
...
test('filtro de período exclui pedido fora do intervalo, inclui borda exata', () => {
  ...
  const result = productReport(items, orders, {
    productId: 'p1', startDate: '2026-08-01', endDate: '2026-08-05',
  });
  assert.equal(result.totalPecas, 2 + 4);
});

test('produto sem venda no período devolve zeros e breakdown vazio', () => {
  const result = productReport([item({ order_id: 'o1' })], [order({ id: 'o1' })], { productId: 'produto-inexistente' });
  assert.deepEqual(result, { totalPecas: 0, totalReceita: 0, breakdown: [] });
});

test('sem productId devolve zerado sem lançar erro', () => {

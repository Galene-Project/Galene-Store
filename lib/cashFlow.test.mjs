...
test('fluxoCaixa: pedido pago dentro do período vira entrada', () => {
  const orders = [
    { id: 'o1', order_number: 'PED-0001', total: 300, paid_at: '2026-08-10T12:00:00Z', status: 'pago', customers: { name: 'Loja A' } },
    { id: 'o2', order_number: 'PED-0002', total: 500, paid_at: '2026-07-30T12:00:00Z', status: 'pago', customers: { name: 'Loja B' } },
    { id: 'o3', order_number: 'PED-0003', total: 100, paid_at: null, status: 'pago', customers: { name: 'Loja C' } },
  ];
  ...
  assert.equal(r.movimentos[0].data, '2026-08-10');
});

test('fluxoCaixa: pedido cancelado não conta como entrada mesmo com paid_at no período', () => {
  const orders = [
    { id: 'o1', order_number: 'PED-0001', total: 300, paid_at: '2026-08-10T12:00:00Z', status: 'pago', customers: { name: 'Loja A' } },
    { id: 'o2', order_number: 'PED-0002', total: 900, paid_at: '2026-08-12T12:00:00Z', status: 'cancelado', customers: { name: 'Loja B' } },
  ];
  const r = fluxoCaixa(orders, [], [], periodo);
  assert.equal(r.totalEntrada, 300);
  assert.equal(r.movimentos.length, 1);
  assert.ok(!r.movimentos.some((m) => m.descricao.includes('PED-0002')));
});
...
test('fluxoCaixa: saldo é entrada menos saída', () => {
  const orders = [{ id: 'o1', order_number: 'PED-0001', total: 1000, paid_at: '2026-08-10T12:00:00Z', status: 'pago', customers: {} }];
